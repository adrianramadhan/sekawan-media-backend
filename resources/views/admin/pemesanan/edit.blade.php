            ...
            <div class="container mx-auto px-4">
                <form method="POST" action="{{ route('admin.pemesanan.update', $pemesanan) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="datetime-local" id="waktu_mulai" name="waktu_mulai" value="{{ $pemesanan->waktu_mulai->format('Y-m-d\TH:i') }}" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500">
                    </div>

                    <div class="mb-4">
                        <label for="waktu_selesai" class="block mb-2 text-sm font-medium text-gray-900">Waktu Selesai</label>
                        <input type="datetime-local" id="waktu_selesai" name="waktu_selesai" value="{{ $pemesanan->waktu_selesai->format('Y-m-d\TH:i') }}" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500">
                    </div>

                    <div class="mb-4">
                        <label for="tujuan" class="block mb-2 text-sm font-medium text-gray-900">Tujuan</label>
                        <input type="text" id="tujuan" name="tujuan" value="{{ $pemesanan->tujuan }}" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="approver1_id" name="approver1_id" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500">
                            @foreach($approvers as $approver)
                                <option value="{{ $approver->id }}" {{ $pemesanan->persetujuan->where('level', '1')->first()->user_id == $approver->id ? 'selected' : '' }}>{{ $approver->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="approver2_id" name="approver2_id" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500">
                            @foreach($approvers as $approver)
                                <option value="{{ $approver->id }}" {{ $pemesanan->persetujuan->where('level', '2')->first()->user_id == $approver->id ? 'selected' : '' }}>{{ $approver->name }}</option>
                            @endforeach
                        </select>
                    </div>
